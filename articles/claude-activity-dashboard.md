...
このダッシュボードのもうひとつの機能が、Claude Codeのスキルとして動作する `/analyze-usage` です。ダッシュボードが「見る」ためのツールなら、このスキルは「改善する」ためのツールです。
...
たとえば「ツール効率」では、`cat`や`grep`、`find`などBashで実行されたが`Read`/`Grep`/`Glob`で代替可能なコマンドを検出します。「指示パターン」では、3回以上繰り返されたユーザー指示を抽出し、スキル化の候補を見つけます。
...
1. **CLAUDE.mdへの追加ルール** — Bash代替パターンが多ければ「専用ツールを使う」ルールを、繰り返し指示が多ければそれを減らすルールを、CLAUDE.mdに追記すべき具体的なテキストとして提案する
2. **新規スキル候補** — 繰り返し実行している操作やツールの連続使用パターンから、スキルとして切り出すべき操作を提案する
3. **Claudeの振る舞いルール** — 意図ズレや修正パターンから、「曖昧な指示を受けたら着手前に確認する」「否定形の指示は肯定形に言い換えて確認する」といったルールをCLAUDE.mdに追記する形で提案する

提案は番号付きで出力され、ユーザーが番号を選択すると対象ファイルに自動反映されます。つまり、**ログ分析 → 改善提案 → CLAUDE.mdやスキルへの反映**がワンストップで完結します。
...
Claude Codeの活動ログには、セッション中のやり取りがすべて記録されています。つまり、会話中にAPIキーや認証情報を扱っていた場合、それらもSQLiteに格納されます。

このダッシュボードはローカル利用を前提としています。Datasetteは `--host` を指定しない限り `127.0.0.1`（localhost）にバインドされるため、同一マシン以外からはアクセスできません。ただし以下の点には注意してください。

- **SQLiteファイル（`data/claude_activity.db`）を外部に公開・共有しない** — ログの全文が格納されている
- **Datasetteを `--host 0.0.0.0` で起動しない** — 同一ネットワーク内の他端末からアクセス可能になる
- **スクリーンショットの共有時にも注意** — セッション内容のプレビューに機密情報が含まれうる
