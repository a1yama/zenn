...
**カスタマイズしてもこの構造は変わりません。** Prefixキー自体を押しやすいキーに変更したり、コマンドをVim風にしたりすることはできますが、「Prefix → コマンド」という2ステップは必ず必要です。これはtmuxを使う上で避けられないデメリットです。
...
tmuxのキーストローク問題は、設定で大幅に改善できます。設定ファイルは以下のリポジトリで公開しています。
